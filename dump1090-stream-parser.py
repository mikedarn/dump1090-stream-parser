...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=BATCH_SIZE,
        help="An integer of the number of rows to write to the database at a time. If you turn off WAL mode, a lower "
             "number makes it more likely that your database will be locked when you try to query it. Defaults to %s"
        % (BATCH_SIZE,),
    )
    parser.add_argument(
        ...
        help="An integer of the number of times to try (and fail) to connect to the dump1090 broadcast before "
             "quitting. Defaults to %s"
    ...
    cur.execute(
        """
            CREATE VIEW IF NOT EXISTS callsigns AS
              SELECT callsign, hex_ident, date(parsed_time) date_seen, max(parsed_time) last_seen, min(parsed_time) 
                     first_seen
                FROM squitters
                WHERE callsign != ""
    ...
    while count_failed_connection_attempts < args.connect_attempt_limit:
        try:
            ...
            count_failed_connection_attempts += 1
            print(
                "Cannot connect to dump1090 broadcast. Making attempt %s."
            ...
            if len(message) == 0:
                ...
                while count_failed_connection_attempts < args.connect_attempt_limit:
                    try:
                        ...
                        count_failed_connection_attempts += 1
                        print(
                            "The attempt failed. Making attempt %s."
                            % count_failed_connection_attempts
                        )
                        time.sleep(args.connect_attempt_delay)
                else:
                    ...
                    try:
                        ...
                        if count_since_commit % args.batch_size == 0:
                            ...
                            if cur_time != last_time:
                                print(
                                    "averaging %s rows per second, currently %s rows per second"
                                    % (
                                        float(count_total)
                            ...
                            if count_since_commit > args.batch_size:
                                print(
                                    ts,
                                    "All caught up, %s rows, successfully written to database"
                                    % count_since_commit,
                                )
                            count_since_commit = 0
                            last_time = cur_time
                        ...
                        print(
                            ts,
                            "Could not write to database, will try to insert %s rows on next commit"
                            % (count_since_commit + args.batch_size,),
                        )
